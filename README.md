...
- 身份与隔离：部署后的访问使用平台提供的 ChatGPT 身份；每个请求都在服务端注入 `tenant_id` 与 `user_id` 条件。本机提供注册、登录和退出的体验账号，密码使用 PBKDF2 派生后保存、会话使用 HttpOnly cookie，便于验证多用户数据隔离；该机制只用于 localhost，不能用于生产认证。
