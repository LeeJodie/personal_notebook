...
默认“本地模式”使用私有 MeloTTS，正文不会发送给外部语音服务。用户可主动选择“联网模式”，此时适配层会使用 EdgeTTS 的在线服务并将当前朗读片段发送给 Microsoft；联网模式提供晓晓、晓伊、云希和云健等中文神经音色。部署时需额外安装 `edge-tts`，`./deploy.sh` 与 Dockerfile 已包含此依赖。
