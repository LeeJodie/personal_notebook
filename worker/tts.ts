...
export async function listMeloTtsVoices(env: Pick<TtsEnv, "CUSTOMER_HTTP_TTS" | "LOCAL_TTS_URL">, headers = new Headers()): Promise<{ provider: string; model: string | null; items: MeloTtsVoice[] }> {
...
export async function synthesizeMeloTts(env: Pick<TtsEnv, "CUSTOMER_HTTP_TTS" | "LOCAL_TTS_URL">, payload: { text: string; voiceId: string; speed: number }, headers = new Headers(), signal?: AbortSignal): Promise<Response> {
...
export async function handleTtsRequest(request: Request, env: TtsEnv): Promise<Response> {
  ...
  if (request.method === "GET" && url.pathname === "/api/tts/voices") {
    try {
      return json(await listMeloTtsVoices(env, headers));
    } catch (error) {
  ...
  if (request.method === "POST" && url.pathname === "/api/tts/synthesize") {
    ...
    try {
      return await synthesizeMeloTts(env, { text, voiceId, speed }, headers, request.signal);
    } catch (error) {
      return problem(error instanceof Error ? error.message : "私有 TTS 服务不可用。", 503, "TTS_UNAVAILABLE");
    }
  }
