...
function markdownStartsParagraph(previous: string, current: string): boolean {
  // Policy Markdown exports often omit blank lines between numbered questions
  // and answers. Preserve those logical line breaks without splitting normal
  // soft-wrapped Markdown lines.
  if (/^(?:[一二三四五六七八九十百零〇]+、|\d+[.、]|[问答][：:])/.test(current)) return true;
...
export function createMarkdownReader(markdown: string, title: string, sourceUrl = "", uploadedSiteName = "上传的 Markdown 文档"): ReaderDocument {
  ...
  for (const rawLine of lines) {
    ...
    const content = line.replace(/^[-*+]\s+/, "• ").replace(/[*_`]/g, "");
    if (buffer.length && markdownStartsParagraph(buffer[buffer.length - 1], content)) flushParagraph();
    buffer.push(content);
  }
  flushSection();
